...
def category_list(request):
	...
	return render_to_response(
		...
		{'categories': categories,'header':header, 'perms': list_perms(request)},
...
def my_topic_list(request, show_user=False):
	...
	* show_user - if not set will show current user topics
	"""
	if not show_user:
		show_user = str(request.user.get_full_name())
	if request.user.is_authenticated():
		...
		return render_to_response(
			...
			{'topics': topics,'header':header, 'forum_name': forum_name, 'perms': list_perms(request)},
			context_instance=RequestContext(request))
	else:
		return render_to_response('bug.html', {'bug': _('You aren\'t logged in')}, context_instance=RequestContext(request))
...
def my_posttopic_list(request, show_user=False):
	...
	* show_user - if not set will show current user topics
	...
	try:
		...
		return render_to_response('myghtyboard/mytopics_list.html', {'perms': list_perms(request),'header':header}, context_instance=RequestContext(request))
	return render_to_response(
		'myghtyboard/mytopics_list.html',
		{'topics': topics,'header':header, 'forum_name': forum_name, 'perms': list_perms(request)},
		context_instance=RequestContext(request))
	...
	* topic_id - id of a Topic entry
	...
	return object_list(
		...
		extra_context = {
			...
			'current_user': str(request.user.get_full_name())},
...
def delete_post(request, post_id, topic_id):
	...
	* topic_id - Topic entry ID that contain the Post entry
	...
	if request.user.is_authenticated() and request.user.is_staff:
		...
		return render_to_response('bug.html', {'bug': _('You aren\'t a moderator')}, context_instance=RequestContext(request))
	...
	if request.user.is_authenticated() and request.user.is_staff:
		if request.POST and len(request.POST['forum']) > 0:
			...
			return render_to_response(
				...
				{'forums': forums,'header':header, 'topic': topic, 'perms': list_perms(request)},
				context_instance=RequestContext(request))
	else:
		return render_to_response('bug.html', {'bug': _('You aren\'t a moderator')}, context_instance=RequestContext(request)) # can't move
...
def open_topic(request, topic_id, forum_id):
	...
	* forum_id - ID of a Forum entry that contain the Topic entry
	...
	if request.user.is_authenticated() and request.user.is_staff:
		...
		return render_to_response('bug.html', {'bug': _('You aren\'t a moderator and you aren\'t logged in')}, context_instance=RequestContext(request))
